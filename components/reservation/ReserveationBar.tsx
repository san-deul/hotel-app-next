...
export default function ReservationBar() {
  ...
  // window는 서버에 없으므로 useEffect에서 안전하게 판단 (SSR 하이드레이션 에러 방지)
  ...
  const startDate = range[0].startDate as Date;
  const endDate = range[0].endDate as Date;
  ...
  const isSelected = (day: Date) => day >= startDate && day <= endDate;

  // 달력 호출 함수
  const customDayRenderer = (day: Date) => {
    // day : 달력 라이브러리가 만들어서 넘겨준 Date 객체
