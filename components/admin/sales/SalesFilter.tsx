...
import type { RangeType } from "@/lib/api/sales";
    ...
    <div className="bg-white p-4 rounded-lg border space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <button
            onClick={onToday}
            className={`${baseBtn} ${rangeType === "today" ? active : inactive}`}
          >
            오늘
          </button>
          <button
            onClick={onWeek}
            className={`${baseBtn} ${rangeType === "week" ? active : inactive}`}
          >
            이번주
          </button>
          <button
            onClick={onMonth}
            className={`${baseBtn} ${rangeType === "month" ? active : inactive}`}
          >
            이번달
          </button>
        </div>
