import type { SalesSummary as SalesSummaryType } from "@/lib/api/sales";
...
export default function SalesSummary({ periodLabel, summary }: SalesSummaryProps) {
  const items = [
      ...
      <div className="grid grid-cols-5 gap-4">
        {items.map((item) => (
          <div key={item.label} className="border rounded-lg p-4">
            ...
            <p className={`text-xl font-bold ${item.danger ? "text-red-500" : ""}`}>
              {item.value}
            </p>
          </div>
        ))}
