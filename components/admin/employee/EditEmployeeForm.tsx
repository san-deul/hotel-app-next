...
export default function EditEmployeeForm({ employee }: { employee: Employee }) {
  ...
  const normalizedBirth = employee.birth
    ? employee.birth.replace(/[^0-9]/g, "").slice(0, 8)
    : "";
    ...
    <div className="p-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded shadow p-8 w-full max-w-3xl">
        <div className="mb-4">
          ...
          <input {...register("email")} disabled className="border rounded w-full p-2 bg-gray-100" />
        </div>
