...
export default function LoginPage() {
  ...
  const onSubmit = async ({ email, password }: LoginFormValues) => {
    ...
    console.log('currentUser after setUser:', currentUser)
    console.log('role check:', currentUser?.role ? ['admin', 'manager'].includes(currentUser.role) : false)

    alert(`${data.user.email} 님, 환영합니다!`)
    // 리다이렉트는 위 useEffect가 user/role 갱신을 감지해서 처림함
  }
